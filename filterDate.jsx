...
function FilterDate({options,cancel}) {
  ...
  const handleDayScroll = useCallback((event) => {
    if (event.deltaY > 0) { // Scrolling down/forward
      setDay(d => (d === daysInMonth ? 1 : d + 1));
    }
    if (event.deltaY < 0) { // Scrolling up/backward
      setDay(d => (d === 1 ? daysInMonth : d - 1));
    }
  }, [daysInMonth]); // Re-create function only if daysInMonth changes

  // Corrected logic for scrolling months.
  const handleMonthScroll = useCallback((event) => {
  ...
  return (
    <>
      ...
      <div className={` grid h-28 gap-2 ${options==3?'grid-cols-3':'' } ${options==2?'grid-cols-2':'' } ${'grid-cols-1' && options==1} text-center `}>
          ...
          {options==3 && <div onClick={()=>setDay(day==1?daysInMonth:day-1)} className="font-light text-sm cursor-pointer  rounded ">
            {day==1?daysInMonth:day-1}
          </div>}
          {options>=2 && <div onClick={()=>setMonth(months[(months.indexOf(month)+12-1)%12])} className="font-light text-sm cursor-pointer  rounded ">
            {months[(months.indexOf(month)+12-1)%12]}
          </div>}
          {(options>=1) &&<div onClick={()=>setYear(year-1)} className=" cursor-pointer text-sm font-light rounded ">
            {year-1}
          </div>}
          
         { options==3 && <div onWheel={handleDayScroll} className="cursor-pointer  border-y-2 border-gray-300  ">
            {day}
          </div>}
          {options>=2 && <div onWheel={handleMonthScroll} className="cursor-pointer  border-y-2 border-gray-300 ">
            {month}
          </div>}
          {options>=1 && <div onWheel={handleYearScroll} className="cursor-pointer   border-y-2 border-gray-300 ">
            {year}
          </div>}
          
          {options==3 && <div onClick={()=>setDay(day==daysInMonth?1:day+1)} className="font-light text-sm cursor-pointer rounded ">
            {day==daysInMonth?1:day+1}
          </div>}
          {options>=2 && <div onClick={()=>setMonth(months[(months.indexOf(month)+1)%12])} className="font-light text-sm cursor-pointer rounded ">
            {months[(months.indexOf(month)+1)%12]}
          </div>}
          {options>=1 && <div onClick={()=>setYear(year+1)} className="font-light text-sm cursor-pointer rounded ">
            {year+1}
          </div>}
      </div>
      <div className='flex w-full pt-3 justify-around'>
          <button onClick={()=>cancel(0)} className='px-4 py-1  text-white  bg-[#f89320] hover:bg-[#f89320]/90 rounded '>
            Cancel
          </button>
