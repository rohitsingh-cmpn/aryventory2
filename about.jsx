...
const data = [
  {
    type: 'intro',
    title: 'The Future of Inventory Management is Here! Transform Your Inventory with Aryventory',
    paragraphs: [
...
function AccordionItem({ item, level = 0 }) {
  ...
  const iconToRender = item.iconImg 
    ...
    : iconMap[item.title] 
      ? <FontAwesomeIcon icon={iconMap[item.title]} className="w-5 h-5 text-gray-600" />
      : null;

  return (
    <div className={`w-full ${level === 0 ? 'bg-white rounded-2xl' : 'mt-2 pl-4 pr-2'}`}>
      <button
      ...
      {(open || level > 0) && (
        <div className="mt-2 space-y-2 pl-4 pr-2">
          ...
          {item.description && level === 0 && <hr className="bg-[#E6E6E6] h-[1px] w-[98%] border-0 mt-2 mb-2" />}
...
function AboutFooter({ footerSection }) {
  return (
    <div className="space-y-1 pt-2 border-t border-gray-300">
      ...
      <img src={shopImg} alt="Aryventory Logo" className="w-30 h-30 object-contain" />
  ...
  return (
    <div className="w-full h-[calc(100vh-120px)] overflow-y-auto p-6 space-y-4 rounded-lg">
      <header className="flex items-end space-x-4 rounded-lg ">
        <img src={shopImg} alt="Aryventory Logo" className="w-30 h-30 object-contain" />
        <h2 className="text-4xl font-bold pb-5 pl-5 text-gray-800">About Aryventory</h2>
      </header>
