...
import { faAngleRight, faArrowRightArrowLeft, faArrowRightFromBracket, faCheckCircle, faCheckSquare, faCircleInfo, faCodeMerge, faFileLines, faHexagonNodes, faHexagonNodesBolt, faInfo, faInfoCircle, faLockOpen, faMessage, faPaintBrush, faPhone, faRightFromBracket, faShield, faShieldHalved, faStar, faUnlockAlt, faUserLock } from '@fortawesome/free-solid-svg-icons'
                ...
                <div className='flex flex-col bg-[#F8F9FD] rounded-2xl'>
                    <div>
                        <NavLink to={content[0].NavLink}>
                            <div onClick={handleMenu} className='flex cursor-pointer flex-row items-center bg-[#F8F9FD] p-3 hover:bg-[#FFC467] rounded-2xl'>
                            <div className=''>
                            ...
                            <div className='flex flex-row justify-between w-full'>
                                ...
                                <div>
                                    <FontAwesomeIcon size='lg' className={`transition-transform duration-300 ${isMenuOpened ? '-rotate-90' : 'rotate-0'}`} icon={faAngleRight} />
                                </div>
                            </div>
                        </div>
                        </NavLink>
                        {!isMenuOpened && <hr className='text-[#EFEFF0]' />}
                        <div className={`transition-all duration-700 ease-in-out overflow-hidden  border-[#EFEFF0] rounded-2xl pl-4 ${isMenuOpened ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
      ...
      {i < arr.length - 1 && <hr className='text-[#EFEFF0]' />}
                ...
                {content.slice(1, 3).map((item,i,arr) => (
                    <NavLink onClick={navigate} key={i} to={item.NavLink} >
                     <div key={i} className='flex flex-row items-center bg-[#F8F9FD]  p-3 hover:bg-[#FFC467] rounded-2xl'>
                        <div className=''>
                            <FontAwesomeIcon size='lg' icon={item.icon} />
                        </div>
                        <div className='flex  flex-row justify-between w-full'>
                ...
                {content.slice(8,9).map((item, i,arr) => (
                    <NavLink onClick={navigate} key={i} to={item.NavLink} >
                     <div key={i} className='flex flex-row items-center bg-[#F8F9FD] p-3 hover:bg-[#F8F9FD] rounded-2xl'>
                        <div className=''>
                            <FontAwesomeIcon size='lg' icon={item.icon} />
                        </div>
                        <div className='flex flex-row justify-between w-full'>
  ...
  return (
    ...
    <div className='flex flex-col w-full  rounded-2xl h-[calc(100vh-65px)]  duration-300 ease-in-out'>
        <div className='flex flex-row w-full p-4 h-full gap-5 overflow-hidden'>
            <div className={` sm:block sm:flex-1/5  ${currentPage ? 'flex-1 ' : ' hidden'} ease-in-out rounded-2xl `}>     
                <SettingSidebar />
            </div>
            <div  className={` sm:block  ${`sm:`}  ${currentPage ? ' hidden' : ' flex-1'} sm:flex-4/5  shadow-md`}>
                <Outlet />
